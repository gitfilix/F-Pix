...
var app = function() {
    ...
    function lightup(){
        ...
        $('.center').delay(500).queue(function (goNext) {
            $(this).animate({
                'opacity': '1'
            },1200);
            goNext();
        ...
        $('.center h2').delay(2200).queue(function (NextStep){
            $(this).animate({
                'letter-spacing':'4px'
            });
            NextStep();
    ...
    var attachNavigEvents = function () {
        $('#ContainerNav a.menulink').on('click', function (e) {
            e.preventDefault();
            // click event auf navig- elemente with hashNavigation
    ...
    function browserfeaturetest(){
        if(essentialfeatures){
            ...
            $.each(features, function(i){
                $('#showTests').append('<li class="fea-list">'+features[i]+'  checked : ' + featurestest[i]+'</li>')
                });
            showOKfeatures();
        }else{
           // alert(1);
    ...
    function showOKfeatures(){
        ...
        //Works
    // if all essntial browserfeatures are true - continue loading
    //function browserfeaturetest(){
    //    if(essentialfeatures){
    //        $('#showTests').html("<li>features</li>");
    //        $.each(features, function(i){
    //            $('#showTests').append('<li>'+features[i]+':' + featurestest[i]+'</li>');
    //        });
    //    }else{
    //        // alert(1);
    //        console.log("featureTest NOT passed");
    ...
    var init = function () {
        ...
        includeView('splashView', 'splash', function (){
            ...
            getPagesfromServer(function(){
                ...
                includeThemeToggler();
                //global var checker: ask if initState ===3, every 100ms, if so, start postInit-function
                checker = setInterval(function(){
                    if(initState==3){
                        // post init function: finish
                        postInit()
                    }
                },100);
            });
    ...
    // 1.1 get data Model (pages in JSON format) from server
    function getPagesfromServer(callback) {
        ...
        $.getJSON('pages.json', function (data){
            ...
            if (callback && typeof(callback) === "function") {
                ...
                callback();
                console.log('getPagesfromServer finished. Pages loaded');
            }
        });
        // if callback ok - continue
    }

    // 3. DOM is ready now - attach Events to components in postInit-function
    function postInit(){
        ...
        // fade out function splash-screen after 2 sec
        setTimeout(function () {
            $('#splash').fadeOut(330, function () {
